...
const HomePricing = () => {
  ...
  const getButtonStyle = useCallback((tierName: string, isPopular: boolean): string => {
    if (isPopular) {
      return 'bg-gradient-to-r from-purple-600 to-purple-700 text-white hover:from-purple-700 hover:to-purple-800 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all duration-200';
    }
    
    switch (tierName.toLowerCase()) {
      case 'starter':
        return 'bg-blue-600 text-white hover:bg-blue-700 shadow-md hover:shadow-lg';
      case 'growth':
        return 'bg-purple-600 text-white hover:bg-purple-700 shadow-md hover:shadow-lg';
      case 'premium':
        return 'bg-amber-600 text-white hover:bg-amber-700 shadow-md hover:shadow-lg';
      default:
...
}, [isAuthenticated, router, user?.id, user?.email]); // DODAJ user?.email U DEPENDENCY
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center">
          <div className="animate-pulse">
            ...
            <div className="h-4 bg-gray-300 rounded w-1/2 mx-auto mb-16" />
    ...
    tiers.map((tier) => (
      <Card
        ...
        className={`relative flex flex-col border-2 transition-all duration-300 hover:shadow-xl ${
          tier.popular 
            ? 'scale-105 border-purple-500 shadow-2xl' 
            : 'border-gray-200 shadow-md hover:scale-[1.02]'
        } ${getTierGradient(tier.name)}`}
      >
        {tier.popular && (
          <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 px-6 py-2 bg-gradient-to-r from-purple-600 to-purple-700 text-white text-sm font-semibold rounded-full shadow-lg">
            ⭐ Most Popular
          </div>
        )}
