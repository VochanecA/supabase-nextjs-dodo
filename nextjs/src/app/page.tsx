// app/page.tsx - LIGHTHOUSE 95+ OPTIMIZED
'use client';

import React, { useState, useEffect, memo, Suspense, useMemo } from 'react';
...
// Dynamic imports sa loading states
const AuthAwareButtons = dynamic(() => import('@/components/AuthAwareButtons'), {
  ssr: false,
  loading: () => null // Remove skeleton to prevent CLS
});

const HomePricing = dynamic(() => import('@/components/HomePricing'), {
  ssr: false, // Change to false to reduce initial bundle
  loading: () => null
});
...
// Memoize static data
const features = [
  { icon: Shield, title: 'Robust Authentication', description: 'Enterprise-grade security with MFA, SSO, and passwordless login', color: 'from-emerald-500 to-teal-600', bg: 'bg-emerald-50 dark:bg-emerald-950' },
...
const MobileMenu = memo(({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 md:hidden">
      ...
      <nav className="absolute right-0 top-0 h-full w-full max-w-sm bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl shadow-2xl border-l border-gray-200/50 dark:border-gray-700/50">
        <div className="flex flex-col h-full">
          <div className="flex items-center justify-between p-5 border-b border-gray-200/50 dark:border-gray-700/50">
            <Link href="/" className="text-2xl font-bold bg-gradient-to-r from-primary-600 to-primary-500 dark:from-primary-400 dark:to-primary-300 bg-clip-text text-transparent" onClick={onClose}>
              {productName}
            </Link>
            <div className="flex items-center gap-2">
              {mounted && (
                <button onClick={toggleTheme} className="p-2.5 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors" aria-label={isDark ? "Light mode" : "Dark mode"}>
                  {isDark ? <Sun className="h-5 w-5 text-amber-500" /> : <Moon className="h-5 w-5 text-indigo-600" />}
                </button>
              )}
              <button onClick={onClose} className="p-2.5 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors" aria-label="Close">
                <X className="h-6 w-6" />
              </button>
            </div>
          </div>
          <div className="flex-1 p-5 overflow-y-auto">
            <div className="space-y-2">
              {menuItems.map((item, i) => {
                ...
                const content = (
                  <div className="flex items-center gap-3 p-4 rounded-2xl hover:bg-primary-50 dark:hover:bg-primary-950 transition-colors">
                    <div className={`p-2.5 rounded-xl bg-gray-100 dark:bg-gray-800 ${item.iconColor}`}>
...
const HeroSection = memo(() => (
  <section className="relative pt-20 pb-16 sm:pt-24 sm:pb-20 md:pt-32 md:pb-28 overflow-hidden" style={{ minHeight: '600px' }}>
    ...
    <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="text-center max-w-4xl mx-auto">
        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r from-primary-100 to-purple-100 dark:from-primary-900/50 dark:to-purple-900/50 border border-primary-200 dark:border-primary-800 mb-6 sm:mb-8">
          <Sparkles className="h-4 w-4 text-primary-600 dark:text-primary-400" />
          <span className="text-xs sm:text-sm font-semibold text-primary-700 dark:text-primary-300">Launch in minutes</span>
        </div>
        ...
        <p className="mt-6 sm:mt-8 text-base sm:text-lg md:text-xl text-gray-700 dark:text-gray-300 max-w-2xl mx-auto leading-relaxed">
          Launch your SaaS product in <strong className="text-primary-600 dark:text-primary-400">days</strong>, not months. Complete with authentication and enterprise-grade security.
        </p>
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="grid grid-cols-2 gap-3 sm:gap-4 md:grid-cols-4 md:gap-6">
        {stats.map((stat, i) => {
          ...
          return (
            <div key={i} className="p-4 sm:p-6 rounded-2xl bg-gradient-to-br from-gray-50 to-white dark:from-gray-800 dark:to-gray-900 border border-gray-200 dark:border-gray-700">
              <Icon className="h-5 w-5 sm:h-6 sm:w-6 text-primary-600 dark:text-primary-400 mb-2 sm:mb-3" />
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="text-center mb-12 sm:mb-16 md:mb-20">
        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary-100 dark:bg-primary-900/50 border border-primary-200 dark:border-primary-800 mb-4">
          ...
          <span className="text-xs sm:text-sm font-semibold text-primary-700 dark:text-primary-300">Features</span>
        </div>
        ...
        <p className="mt-4 sm:mt-6 text-base sm:text-lg text-gray-600 dark:text-gray-300 max-w-2xl mx-auto">
          Built with modern tech for reliability and speed
        </p>
      </div>
      ...
      <div className="grid gap-4 sm:gap-6 md:gap-8 md:grid-cols-2 lg:grid-cols-3">
        {features.map((feature, i) => {
          ...
          return (
            <article
              key={i}
              className="p-6 sm:p-8 rounded-3xl bg-white/80 dark:bg-gray-900/80 border border-gray-200 dark:border-gray-700"
            >
...
const CTASection = memo(() => (
  <section className="relative py-16 sm:py-20 md:py-28 overflow-hidden">
    ...
    <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
      ...
      <p className="mt-6 sm:mt-8 text-base sm:text-lg md:text-xl text-white/90 max-w-2xl mx-auto leading-relaxed">
        Join thousands building with <strong className="text-white">{productName}</strong>
      </p>

      <div className="mt-8 sm:mt-10 md:mt-12 flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4">
        <Link
          href="/auth/register"
          className="w-full sm:w-auto inline-flex items-center justify-center px-8 py-4 rounded-2xl bg-white text-gray-900 font-bold hover:bg-gray-50 transition-colors shadow-2xl text-base md:text-lg"
        >
          Get Started Free
        ...
        <Link
          href="/demo"
          className="w-full sm:w-auto inline-flex items-center justify-center px-8 py-4 rounded-2xl bg-white/10 border-2 border-white/50 text-white font-bold hover:bg-white/20 transition-colors text-base md:text-lg"
        >
          Watch Demo
      ...
      <div className="grid grid-cols-2 gap-8 sm:gap-12 md:grid-cols-4">
        <div className="col-span-2 md:col-span-1">
          ...
          <p className="mt-4 text-sm text-gray-600 dark:text-gray-400">
            Launch your SaaS in minutes with enterprise infrastructure.
          </p>
        </div>
        ...
        <div>
          ...
          <ul className="space-y-3">
            <li><Link href="https://github.com/Razikus/supabase-nextjs-template" className="text-sm text-gray-600 dark:text-gray-400 hover:text-primary-600 dark:hover:text-primary-400 transition-colors" target="_blank" rel="noopener noreferrer">Docs</Link></li>
          </ul>
        </div>
        ...
        <div>
          ...
          <ul className="space-y-3">
            <li><Link href="/legal/privacy" className="text-sm text-gray-600 dark:text-gray-400 hover:text-primary-600 dark:hover:text-primary-400 transition-colors">Privacy</Link></li>
            <li><Link href="/legal/terms" className="text-sm text-gray-600 dark:text-gray-400 hover:text-primary-600 dark:hover:text-primary-400 transition-colors">Terms</Link></li>
          </ul>
        </div>
      </div>
...
// Navigation - Optimized with scroll detection
const Navigation = memo(() => {
  ...
  return (
    <>
      <nav className={`fixed top-0 w-full z-40 transition-all duration-300 ${scrolled ? 'bg-white/90 dark:bg-gray-900/90 backdrop-blur-xl shadow-lg border-b border-gray-200/50 dark:border-gray-700/50' : 'bg-transparent'}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16 items-center">
            <Link href="/" className="text-xl sm:text-2xl font-bold bg-gradient-to-r from-primary-600 to-purple-600 dark:from-primary-400 dark:to-purple-400 bg-clip-text text-transparent">
              {productName}
            </Link>
            ...
            <div className="hidden md:flex items-center gap-2 lg:gap-4">
              {menuItems.map((item, i) => (
                <Link
                  key={i}
                  href={item.href}
                  className="px-4 py-2 rounded-xl text-gray-700 hover:text-primary-600 dark:text-gray-300 dark:hover:text-primary-400 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors font-medium text-sm lg:text-base"
            ...
            <div className="flex items-center gap-2 md:hidden">
              ...
              <button className="p-2.5 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors" onClick={() => setIsMobileMenuOpen(true)} aria-label="Menu">
